...
router.get("/", async (req, res) => {
  try {
    ...
    const announcementsWithParsedImages = publishedAnnouncements.map(announcement => ({
      ...announcement,
      images: announcement.images ? JSON.parse(announcement.images as string) : []
    }));

    res.json(announcementsWithParsedImages);
  } catch (error) {
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const announcementWithParsedImages = {
      ...announcement[0],
      images: announcement[0].images ? JSON.parse(announcement[0].images as string) : []
    };
